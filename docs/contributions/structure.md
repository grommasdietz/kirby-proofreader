...
- `lib/` (PHP): add classes under the `GrommasDietz\\Proofreader\\` namespace.
...
Psalm errors if directories referenced in `issueHandlers` don't exist. The
configuration ships with global suppressions for common Kirby patterns, plus
